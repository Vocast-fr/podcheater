...
const waitUntilConnectionIsUp = async () => {
  let connected = false
  while (!connected) {
    try {
      await checkInternetConnected({
        timeout: 4000, // timeout connecting to each server(A and AAAA), each try (default 5000)
        retries: 10, // number of retries to do before failing (default 5)
